...
while read HOST ID
do
	RES=$(ssh $HOST "egrep -i '^[^[:print:]]*(java\.|assert|terminate|exception)' /tmp/jpaxos_${ID}/log.* | grep -v 'Connection reset by peer' | sort | uniq" </dev/null)
	if [ "$RES" ]
	then
		echo -e "\n\033[01;31mErrors in log files\033[00m\n"
		echo "$RES"
		exit -1
	fi
done
