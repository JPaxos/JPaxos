...
public:
    ServiceProxyStorage() {startingSeqNo.push_back(*pop, std::pair<jint, jint>(0,0));}
    ...
    void appendStartingSeqenceNo(jint instance, jint sequenceNo) {startingSeqNo.push_back(*pop, std::pair<jint, jint>(instance, sequenceNo));}
    void truncateStartingSeqNo(int lowestSeqNo) {
        pmem::obj::transaction::run(*pop,[&]{
            while(true){
                ...
                ++it;
                auto & nextSn = it->get_ro().second;
                
                if(nextSn > lowestSeqNo)
                    return;
                
                startingSeqNo.pop_front(*pop);
            }
        });        
    }
    void clearStartingSeqNo() {startingSeqNo.clear(*pop);}
    jintArray getFrontStartingSeqNo(JNIEnv * env) {
        jintArray jia = env->NewIntArray(2);
        jint* ia = env->GetIntArrayElements(jia, nullptr);
        auto & front =  startingSeqNo.begin()->get_ro();
        ia[0] = front.first;
        ia[1] = front.second;
