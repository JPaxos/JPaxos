...
/**
 * Simple implementation of a latency detector.
 * The LD periodically sends ping pong messages to all the processes 
 * and compute the RTT.
 * The variable NB_DROP_ALLOWED correspond to the number of consecutive 
 * non-response allowed before the LD report an infinite RTT value.
 * 
 * @author Benjamin Donzé
 * @author Nuno Santos
 */
public class SimpleLatencyDetector implements LatencyDetector {
	...
	/** How long the latency detector waits until sending ping message. In milliseconds */
	...
	/** The maximum duration of a round trip. In milliseconds */
	private final int MAX_RTT = 1500;	
	...
	/* Adding and removing elements from the listener arrays may be performed
	 * by external threads, while iterating is done by the dispatcher thread.
	...
	public SimpleLatencyDetector(ProcessDescriptor p, Network network, SingleThreadDispatcher executor) {
		this.sendPeriod = p.config.getIntProperty(SEND_PERIOD, DEFAULT_SEND_PERIOD);
		this.delta = p.config.getIntProperty(DELTA, DEFAULT_DELTA);
		
		if (sendPeriod <= 2*delta) {
			throw new IllegalArgumentException("Send period (" + sendPeriod + ") must be bigger than 2*delta: " + (2*delta));
		}
	...
	void onStart() {
		...
		sendPingTask = (ScheduledFuture<SendPingTask>) executor.scheduleAtFixedRate(
				new SendPingTask(), 0, sendPeriod, TimeUnit.MILLISECONDS);
	}
	...
	final class SendPingTask extends Handler {
		public void handle() {
			executor.checkInDispatcher();
			assert notifyTask == null : 
				"Starting a new PING rounds before finishing the previous one";
			...
			// that's the upper estimate of how long it takes to receive all messages
			// Otherwise, there's a big delay between measurement and notification.
			notifyTask = (ScheduledFuture<NotifyListenersTask>) executor.schedule(
					new NotifyListenersTask(), 2*delta, TimeUnit.MILLISECONDS);
		}
	}
	...
	final class NotifyListenersTask extends Handler {
		...
		public void handle() {
			...
			if (_logger.isLoggable(Level.INFO)) {
				...
				Arrays.sort(aux);
				_logger.info("New RTT vector: " + Arrays.toString(rttVector) + ", Maj: " + aux[N/2]);
			}
			
			// BroadcastUpdateTask 
			for (LatencyDetectorListener listener : listeners) {
				listener.onNewRTTVector(Arrays.copyOf(rttVector, rttVector.length));
			}			
			notifyTask = null;
		}
...
//						_logger.fine("Received " + msg " + sender);
					...
					case Pong:
						if(msg.getView() == seqNum) {
							// Convert from nano seconds to milliseconds using doubles
