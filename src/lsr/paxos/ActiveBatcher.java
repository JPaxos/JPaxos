 ...
 * for proposing.
 * 
 *  The Selector thread calls the {@link #enqueueClientRequest(ClientRequest)} method
...
public class ActiveBatcher implements Runnable {
    ...
    private final BlockingQueue<ClientRequest> queue = new ArrayBlockingQueue<ClientRequest>(MAX_QUEUE_SIZE);
    
    private ClientRequest SENTINEL = new ClientRequest(RequestId.NOP, new byte[0]);
...
//        queue.put(request);
    ...
    public void run() {
        ...
        try {
            ...
            ClientRequest overflowRequest = null;
