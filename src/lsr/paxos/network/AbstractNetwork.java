...
public abstract class AbstractNetwork implements Network {
	...
	private final Map<MessageType, CopyOnWriteArrayList<MessageHandler>> _msgListeners = Collections
			.synchronizedMap(new EnumMap<MessageType, CopyOnWriteArrayList<MessageHandler>>(
					MessageType.class));

	public AbstractNetwork() {
		// Initialize the enum map with empty arrays. Simplifies the rest of the
		// code
	...
	protected final void fireReceiveMessage(Message message, int sender) {
		assert message.getType() != MessageType.SENT
				&& message.getType() != MessageType.ANY;
		boolean handled = broadcastToListeners(message.getType(), message,
				sender);
	...
	private final boolean broadcastToListeners(MessageType type, Message msg,
			int sender) {
		List<MessageHandler> handlers = _msgListeners.get(type);
		boolean handled = false;
		// _logger.info(this + " Message: " + msg + " (as "+ type + ") from " +
		// sender);
	 ...
	 *            sent
	 * @param dest
	 *            id's of destinations to which message was sent
	 */
	...
	public void addMessageListener(MessageType mType, MessageHandler handler) {
		...
		CopyOnWriteArrayList<MessageHandler> handlers = _msgListeners
				.get(mType);
	...
	private final static Logger _logger = Logger
			.getLogger(AbstractNetwork.class.getCanonicalName());
}
