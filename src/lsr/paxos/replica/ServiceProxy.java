		...
		replicaDispatcher.executeAndWait(new Runnable() {
			public void run() {
				...
				if (thisInstanceReplies == null) {
					...
					if (!hasLastResponse) {
						if (response == null)
							throw new IllegalArgumentException("If snapshot is executed from within execute()"
									+ " for current request, the response has to be given with snapshot");
						snapshot.partialResponseCache.add(new Reply(currentRequest.getRequestId(), response));
					}
				}
